...
langgraph-email-assistant/
│
├── data/                          # Golden mock emails for evaluation
│
├── results/                       # Output artifacts from evaluation
│   └── ...                        # JSON / metrics from eval runs
│
├── frontend/                      # Streamlit HITL & demo frontend
│   ├── streamlit_app.py           # Main Streamlit entrypoint (Demo + Realtime modes)
│   ├── api_client.py              # HTTP client for FastAPI backend
│   ├── demo_data.py               # Safe demo emails (demo mode only)
│   ├── state_adapter.py           # Backend state → UI-friendly mapping
│   │
│   └── components/                # UI components (pure presentation)
│       ├── email_view.py          # Renders email content
│       ├── draft_view.py          # Renders LLM draft
│       ├── hitl_controls.py       # Approve / Deny / Edit controls
│       ├── logs_view.py           # Execution logs viewer
│       └── memory_view.py         # Persistent memory visualization
│
├── src/                           # Main backend source code
│   │
│   ├── api/                       # FastAPI HTTP route definitions
│   │   ├── __init__.py
│   │   ├── router.py              # /triage/email entrypoint
│   │   ├── hitl_router.py         # HITL decision endpoints
│   │   ├── debug_router.py        # Debug + HITL thread inspection
│   │   ├── eval_router.py         # Evaluation & metrics endpoints
│   │   ├── huggingface_router.py  # Dataset / model demo endpoints
│   │   └── ...
│   │
│   ├── graph/                     # LangGraph workflows and execution engine
│   │   ├── __init__.py
│   │   ├── email_graph.py         # Graph definition (nodes + edges)
│   │   ├── graph_registry.py      # Singleton graph instance manager
│   │   ├── checkpoint.py          # MemorySaver checkpointer setup
│   │   └── ...
│   │
│   ├── state/                     # Typed state definitions
│   │   └── state.py               # EmailState (single source of truth)
│   │
│   ├── agents/                    # Agent reasoning nodes
│   │   ├── react_loop.py          # LLM reasoning + draft generation
│   │   └── triage_node.py         # Triage decision logic
│   │
│   ├── triage/                    # Email classification helpers
│   │   └── ...                    # Keyword heuristics, rules
│   │
│   ├── utils/                     # Reusable utilities
│   │   ├── __init__.py
│   │   ├── logger.py              # Execution logging helper
│   │   ├── enforce.py             # Hard memory enforcement
│   │   └── ...
│   │
│   ├── memory/                    # Persistent user memory
│   │   └── store.py               # SQLite-backed preference store
│   │
│   ├── config/                    # Configuration & safety controls
│   │   ├── tools.py               # Dangerous tool registry
│   │   ├── smith.py               # LangSmith configuration
│   │   └── ...
│   │
│   ├── integrations/              # External service integrations
│   │   ├── gmail.py               # Gmail API helpers
│   │   ├── calendar.py            # Google Calendar helpers
│   │   ├── gmail_webhook.py       # Gmail webhook receiver
│   │   └── ...
│   │
│   └── ...                        # Tests / helpers / extensions
│
├── memory.db                      # Local SQLite memory store (runtime-generated)
├── .gitignore                     # Ignore rules (env, secrets, local artifacts)
├── README.md                      # Project documentation
├── requirements.txt               # Python dependencies
├── run_server.py                  # FastAPI application entrypoint
├── script.py                      # Minimal debug / test runner
└── streamlit_app.py               # (Deprecated) Old UI – replaced by frontend/
```
